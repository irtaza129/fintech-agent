...
class RSSFetcher:
    ...
    def _detect_tickers(self, article: Dict, tickers: set, company_names: Dict) -> List[str]:
        ...
        for ticker in tickers:
            # Look for ticker with $ prefix or surrounded by word boundaries
            pattern = r'(\$' + ticker + r'|\b' + ticker + r'\b)'
            if re.search(pattern, text):
                detected.add(ticker)
        
        # Check for company names with word boundaries to avoid false positives
        # (e.g., "Apple" shouldn't match "pineapple")
